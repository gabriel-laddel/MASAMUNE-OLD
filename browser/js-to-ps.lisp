...
;;; compile, so I've fixed it up into this.
...
(loop for f in browser-files 
      collect (handler-case (progn (js->ps f) f)
		(error nil (list :failed f))))

(loop for f in browser-files
      collect (handler-case (progn (js->ps f) f)
		(error nil (list :failed f))))

(js->psf #P"/root/quicklisp/local-projects/masamune/browser/repl/chrome/content/server.js")

(:FAILED)
(:FAILED #P)
(:FAILED #P"/root/quicklisp/local-projects/masamune/browser/repl/chrome/content/util.js")
(:FAILED #P"/root/quicklisp/local-projects/masamune/browser/default-conkeror.js")
