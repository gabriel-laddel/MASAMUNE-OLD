...
(defun run-or-focus-kmap ()  
  (bt:make-thread (lambda () (if (some (lambda (w) (string= "Kmap" (stumpwm::window-name w))) (stumpwm::all-windows))
			    (stumpwm::select-window "Kmap")
			    (progn (setf *kmap* (make-application-frame 'kmap))
  ...
  (multiple-value-bind (x y) (clim:stream-cursor-position sheet)
	...
	(coerce (slot-value (sheet-region sheet) 'clim-internals::coordinates) 'list)
      (let* ((x (if (node-focusedp node) (/ (- x2 x1) 2) x)) 
	     ...
	     (presentation (with-output-as-presentation (sheet node 'node :allow-sensitive-inferiors nil) 
