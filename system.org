...
# TODO documentation build is broken and I don't know why
# emerge tex-base
# cd /doc/manual && make
export SBCL_HOME=/usr/bin/lib/sbcl && INSTALL_ROOT=/usr/bin sh install.sh
# NOTE: I have /no/ idea what SBCL_HOME && INSTALL_ROOT are really for - ie, why have them at all. I understand that UNIX splits up program install locations in a bunch of locations, but I've never understood why exactly. in any case, just
...
sbcl --load ~/quicklisp/local-projects/masamune/build/pre-emacs-install.lisp
...
human mode: the 'state' your computer is in when the full Masamune graphical environment & associated tooling is running.
